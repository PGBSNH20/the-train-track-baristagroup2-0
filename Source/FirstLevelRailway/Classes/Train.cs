...
namespace FirstLevelRailway
{
    public class Train
    {
        ...
        public List<(IRailwayPart Part, int Ticks)> RailWithTicksBetween(int stationAIndex, int stationBIndex, int diffToAdd)
        {
            ...
            var railsWithTicks = RouteDivider.GetRailsWithTicks(Route[stationAIndex].Ticks + diffToAdd, Route[stationBIndex].Ticks, rails);
            return railsWithTicks;
        }
        public void GetFullRoute()
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                var stationRails = RouteDivider.GetRouteStationRails(Route[i], Route[i + 1].Ticks);
                newList.AddRange(stationRails);
            }
            this.Route = newList;
        }
